...
int main(void)
{
  ...
  MW_usbattach();
  MW_Arduino_Init();
  rtmSetErrorStatus(Rotation_M, 0);
  ...
  errorCode = extmodeInit(Rotation_M->extModeInfo, &rtmGetTFinal(Rotation_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(Rotation_M, true);
    }
  }
  ...
  runModel = !extmodeSimulationComplete() && !extmodeStopRequested() &&
    !rtmGetStopRequested(Rotation_M);
